...
def main():
    ...
    for issue in issues:
        ...
        status_atual = issue.get("fields", {}).get("status", {}).get("name", "")
        ...
        if b_age is not None:
            backlog_age.append({"key": key, "dias": b_age})
        if l_time is not None:
            lead_time.append({"key": key, "dias": l_time})
        if c_time is not None:
            cycle_time.append({"key": key, "dias": c_time})
        if t_refinar is not None:
            tempo_refinar.append({"key": key, "dias": t_refinar})
